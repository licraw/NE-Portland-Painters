...
- Trello key setup: create/open your Power-Up at `https://trello.com/power-ups/admin`, then copy the value from the API Key tab into `TRELLO_API_KEY`. Do not use the Power-Up secret as the API key.
- Trello token setup: generate `TRELLO_API_TOKEN` with `scope=read,write`, e.g. `https://trello.com/1/authorize?expiration=never&scope=read,write&response_type=token&key=YOUR_API_KEY`. Do not use the Power-Up secret; this app does not exchange the secret for a token.
- Trello config check: run `yarn check:trello` after updating `.env.local`.
